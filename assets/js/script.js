...
function criarGraficoPizza(id, labels, dados) {
...
function criarGraficoBarra(id, labels, dados) {
  const el = document.getElementById(id);
  if (!el) return;
...
function renderChartsExecutivos() {
  ...
  const projetosStatus = contarPorStatus(projetos);
  const sessoesStatus = contarPorStatus(sessoes);
  const documentosStatus = contarPorStatus(documentos);

  criarGraficoPizza(
    "chartProjetosStatus",
    Object.keys(projetosStatus),
    Object.values(projetosStatus)
  );

  criarGraficoBarra(
    "chartSessoes",
    Object.keys(sessoesStatus),
    Object.values(sessoesStatus)
  );

  criarGraficoPizza(
    "chartTransparencia",
    Object.keys(documentosStatus),
    Object.values(documentosStatus)
  );

  criarGraficoBarra(
    "chartResumoGeral",
    ["Vereadores", "Projetos", "Sessões", "Documentos"],
    [
      vereadores.length,
      projetos.length,
      sessoes.length,
...
document.addEventListener("DOMContentLoaded", function () {
  setTimeout(renderChartsExecutivos, 500);
});
